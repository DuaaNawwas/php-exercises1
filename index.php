...
<body>
    ...
    function stringLen2($words)
    {
        $lowest = strlen($words[0]);
        $highest = strlen($words[0]);
        foreach ($words as $word) {
            if (strlen($word) > $highest) {
                $highest = strlen($word);
            }
            if (strlen($word) < $lowest) {
                $lowest = strlen($word);
            }
        }
        echo "The shortest array length is " . $lowest . ". The longest array length is " . $highest . ". <br>";
    }
    stringLen2(["abcd", "abc", "", "hjjjsfdsf", "g", "wer"]);
    ...
    lowestInt([2, 0, 10, 12, 6]);

    function lowestInt2($array)
    {

        $lowest = $array[0];
        for ($i = 0; $i < count($array); $i++) {
            if ($array[$i] < $lowest and $array[$i] != 0) {
                $lowest = $array[$i];
            }
        }
